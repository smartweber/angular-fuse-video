...
export class TodoService implements Resolve<any>
{
    ...
    setCurrentTodo(id)
    {
        ...
        let channelId = this.route.pathFromRoot[this.route.pathFromRoot.length-3].params['id'];

        if ( tagHandle )
        {
            this.location.go('pg/channel/' + channelId + '/todo/tag/' + tagHandle + '/' + id);
        }
        else if ( filterHandle )
        {
            this.location.go('pg/channel/' + channelId + '/todo/filter/' + filterHandle + '/' + id);
        }
        else
        {
            this.location.go('pg/channel/' + channelId + '/todo/all/' + id);
        }
